...
add_filter( 'thk_head', function() use( &$luxe ) {
	...
	if( $luxe['html_compress'] !== 'none' && !isset( $luxe['amp'] ) ) {
		// id のついてないインラインスタイルは一つにまとめて圧縮
		preg_match_all( '#(?:(?!<noscript>))<style>([^<]+?)</style>(?:(?!</noscript>))#', $head, $matches );
		if( isset( $matches[1] ) ) {
	...
	if( !empty( $inline_style ) ) {
		...
		if( stripos( $head, '<noscript><style' ) !== false && !isset( $luxe['amp'] ) ) {
			preg_match_all( '#<noscript><style.+?</noscript>#', $head, $matches );

			if( isset( $matches[0] ) ) {
				foreach( (array)$matches[0] as $key => $val ) {
					if( isset( $matches[0][$key] ) ) {
						$head = str_replace( $matches[0][$key], '', $head );
						$noscript_styles .= $val;
					}
