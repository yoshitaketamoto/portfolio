...
?>
<div class="next"><?php next_post_link( '%link', $next_thumb . '<div class="ntitle">' . $next_post->post_title . '</div><div class="next-arrow">' . $awesome['arrow-forward'] . '<span>' . __( '次の記事', 'luxeritas' ) . '</span></div>' ); ?></div>
<?php
	}
	else {
?>
...
?>
<div class="prev"><?php previous_post_link( '%link', $prev_thumb . '<div class="ptitle">' . $prev_post->post_title . '</div><div class="prev-arrow">' . $awesome['arrow-back'] . '<span>' . __( '前の記事', 'luxeritas' ) . '</span></div>' ); ?></div>
<?php
	}
	else {
?>
...
?>
<h2 class="related"><?php echo $awesome['related'], __( 'Related Posts', 'luxeritas' ); ?></h2>
<?php
		if( isset( $luxe['amp'] ) ) {
			ob_start();
